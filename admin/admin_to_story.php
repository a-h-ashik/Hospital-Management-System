<?php
    ...
    if (!isset($_REQUEST["search"])) {
        ...
        $sql = "SELECT * FROM stories WHERE sto_name LIKE '%$value%'";
        $story = mysqli_query($conn, $sql);
    }
    ...
    if (isset($_REQUEST["submit"])) {
        if (($_REQUEST["name"] == "") || ($_REQUEST["details"] == "")) {
            ...
            if (move_uploaded_file($temp_path, $target)) {
                ...
                $sql = "INSERT INTO stories VALUES ('$name', '$details', '$img_name', '$admin_id', '$date')";
                if (mysqli_query($conn, $sql)) {
                    $em = "Account is created successfully.";
                    $css_class = "alert-success";
                    header("Location: ./admin_to_story.php");
                }
                else {
...
<body>
            ...
            <table action="" method="post">
                ...
                <tbody>
                    ...
                    <td><?php echo $row["sto_date"] ?></td>
                    <td class="">
                    <a href="./action/update_story.php?story_name=<?php echo $row["sto_name"] ?>"><button class="button_update" name="update"><span class="material-symbols-outlined">edit</span></button></a>
                    <a href="./action/delete_news_story.php?type=story&name=<?php echo $row["sto_name"] ?>" ><button class="button_delete" name="delete"><span class="material-symbols-outlined">delete_forever</span></button></a>                       
                    </td>
                    </tr>
                    <?php } ?>
                </tbody>
